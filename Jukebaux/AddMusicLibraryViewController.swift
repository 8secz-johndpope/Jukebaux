                ...
                if #available(iOS 10.3, *) {
                    print(element)
                        if let name = element.title,
                            ...
                            if storeID == 0 {
                            self.SharedJukebauxModel.parties[self.SharedJukebauxModel.currentPartyIndex].searchTrackAndGetTrackImageURLandThenAddSong(songName: name, songArtist : artist, songImage: image, completionHandler: {_ in
                                    i+=1
                               ...
                               let songDuration = element.playbackDuration
                                self.SharedJukebauxModel.parties[self.SharedJukebauxModel.currentPartyIndex].getTrackImageURLandThenAddSong(songName: name, songArtist : artist, songID : String(storeID), songImage: image, songDuration: Int(songDuration*1000), completionHandler: {_ in
                                        i+=1
                                        print("added: \(name) : \(i)/\(selectedSongs.count)")
                                        // If all the songs have been added
                                        let minimumDoneSongs = Int(Double(selectedSongs.count) * 1)
