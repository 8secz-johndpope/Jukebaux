...
class PartiesTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        self.navigationController?.navigationBar.barTintColor = SharedJukebauxModel.mainJukebauxColor
    ...
    func removeObservers() {
        if let refHandleAdd = partyHandleAdd {
            ...
            SharedJukebauxModel.ref.child("parties").removeObserver(withHandle: refHandleAdd)
        }
        if let refHandleRemove = partyHandleRemove {
            print("remove parties ref handle remove")
            SharedJukebauxModel.ref.child("parties").removeObserver(withHandle: refHandleRemove)
        }
        if let refHandleObserve = partyHandleModify {
            print("remove parties ref handle change")
            SharedJukebauxModel.ref.child("parties").removeObserver(withHandle: refHandleObserve)
        }
    }
    ...
    func loadPartiesFromFirebase() {
        ...
        //        handle = SharedJukebauxModel.ref.child("parties").observe(DataEventType.value, with: { (snapshot) in
        //            if !snapshot.exists() {
        //                return
        //            }
        //
        ...
        //            }
        //
        //            self.SharedJukebauxModel.parties = newParties
        //
        //            for (index, party) in self.SharedJukebauxModel.parties.enumerated() {
        ...
        //                        if let data1 = data {
        //                            DispatchQueue.main.async {
        //                                self.SharedJukebauxModel.parties[index].image = UIImage(data: data1)!
    ...
    func setPartiesFirebaseObservers() {
        partyHandleAdd = SharedJukebauxModel.ref.child("parties").observe(DataEventType.childAdded, with: { (snapshot) -> Void in
            if !snapshot.exists() {
                return
            }
            if let childPartySnapshot = snapshot as? DataSnapshot {
                ...
                if !self.SharedJukebauxModel.parties.contains(where: { $0.partyID == newParty.partyID}) {
                    self.SharedJukebauxModel.parties.insert(newParty, at: 0)
                    ...
                    if  let url = URL(string: FBSavedImageURL){
                        URLSession.shared.dataTask(with: url, completionHandler: { (data, response, error) in
                            ...
                            if let data1 = data {
                                DispatchQueue.main.async {
                                    //                                    for element in self.SharedJukebauxModel.parties {
                                    //                                        print(element.toAnyObject())
                                    //                                    }
                                    let index = self.SharedJukebauxModel.parties.index(where: {$0.partyID == newParty.partyID})
                                    self.SharedJukebauxModel.parties[index!].image = UIImage(data: data1)!
        ...
        partyHandleRemove = SharedJukebauxModel.ref.child("parties").observe(DataEventType.childRemoved, with: { (snapshot) -> Void in
            ...
            if let childPartySnapshot = snapshot as? DataSnapshot {
                if let childPartyID = (childPartySnapshot.value as! NSDictionary)["partyID"] as? String {
                    if let i = self.SharedJukebauxModel.parties.index(where: { $0.partyID == childPartyID }) {
                        print("observed removed in parties \(self.SharedJukebauxModel.parties[i].partyName)")
                        let rowToDelete = IndexPath.init(row: i, section: 0)
                        let rowsToDelete = [rowToDelete]
                        self.tableView.beginUpdates()
                        self.SharedJukebauxModel.parties.remove(at: i)
        ...
        partyHandleModify = SharedJukebauxModel.ref.child("parties").observe(DataEventType.childChanged, with: { (snapshot) -> Void in
                ...
                if let i = self.SharedJukebauxModel.parties.index(where: { $0.partyID == childPartyID }) {
                    if let partyDictionary = (childPartySnapshot.value as? NSDictionary) {
                        ...
                        if let numberJoined = partyDictionary["numberJoined"] as? Int{
                            self.SharedJukebauxModel.parties[i].numberJoined = numberJoined
                        }
    ...
    func loadData() {
        let activityIndicatorView = NVActivityIndicatorView(frame: self.view.frame, type: .lineScalePulseOut, color: SharedJukebauxModel.mainJukebauxColor, padding: CGFloat(0))
        
        activityIndicatorView.startAnimating()
        SharedJukebauxModel.loadFromFirebase(completionHandler: {_ in
            self.parties = self.SharedJukebauxModel.parties
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if (segue.identifier == "ShowPartySegue") {
            ...
            SharedJukebauxModel.currentPartyIndex = indexPath!.row
            removeObservers()
            //add user to joined in party
            SharedJukebauxModel.ref.child("parties").child(SharedJukebauxModel.parties[SharedJukebauxModel.currentPartyIndex].partyID).child("users").child(SharedJukebauxModel.myUser.username).setValue(SharedJukebauxModel.myUser.username)
            
            //increase the number joined in the party in transaction block to avoid concurrency issues
            SharedJukebauxModel.ref.child("parties").child(SharedJukebauxModel.parties[SharedJukebauxModel.currentPartyIndex].partyID).runTransactionBlock({ (currentData: MutableData) -> TransactionResult in
            ...
            if ( SharedJukebauxModel.myUser.isHost ) {
